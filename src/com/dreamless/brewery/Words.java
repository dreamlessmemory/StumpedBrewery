						...
						if (command.length() + 1 < chat.length()) {
							if (Character.isSpaceChar(chat.charAt(command.length()))) {
								if (chat.toLowerCase().startsWith(command.toLowerCase())) {
									if (log) {
										Brewery.breweryDriver.log(Brewery.getText("Player_TriedToSay", name, chat));
									}
	...
	public static void playerChat(AsyncPlayerChatEvent event) {
		...
		if (bPlayer != null && bPlayer.isDrunkEffects()) {
			if (!words.isEmpty()) {
				...
				if (log) {
					Brewery.breweryDriver.log(Brewery.getText("Player_TriedToSay", event.getPlayer().getName(), message));
				}
